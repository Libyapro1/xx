...
author = 'HENDGAM'
git_hub = 'github.com/muniroh96'
faceb0ok = 'UNKNOWN'
version = 'solo attack v.96'
...
def logo(n):
	return str(f"""
__ __ .-'.' .-. '.'-. .'.(( ( ^ `> )).'. /`'- \'._____\ (_____.'/ -'`\ |-''`.'------' '------'.`''-| |.-'`.'.'.`/ | | \`.'.'.`'-.| \ .' . / | | | | \ . '. / '._. : _|_| |_|_ : ._.' ````` /T"Y"T\ ````` / | | | \ jgs `'`'`'`'`'`
...
try:
	...
	A = ''
	one = ses.get('https://raw.githubusercontent.com/TheSpeedX/PROXY-List/master/socks4.txt').text
